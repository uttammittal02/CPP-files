...
//--------------------------------------------------------------------------------------------------------//
//--------------------------------------------------------------------------------------------------------//
vi primes = sieve((int)100);
vi visited(1000005, 0);
vi divisor(1000005, 1);

void d(int num, int i){
    int x = primes[i];
    if (num*x > 1000000) return;
    // if (visited[num*x]) return;
    divisor[x] = 2;
    if (_gcd(num, x) == 1) divisor[num*x] = (divisor[num] * divisor[x]) % MOD;
    else{
        int cnt = 0;
        while(!(num % x)){
            num/=x;
        ...
        num *= pow(x, cnt);
        divisor[num*x] = (divisor[num] / (cnt + 1) * (cnt + 2)) % MOD;
    }
    // cout<<num<<space<<x<<" divisors "<<divisor[num*x]<<endl;
    loop(j, i, sz(primes)){
        d(num*x, j);
    }
    // visited[num*x]++;
...
int32_t main(){
    ...
    // test_cases_loop{
        ...
        loop(i, 1, a+1){
            loop(j,1, b+1){
                loop(k, 1, c+1){
                    ans= (ans + divisor[i*j*k]) % MOD;
                    // cout<<i*j*k<<" debug "<<divisor[i*j*k]<<endl;
                }
            }
        }
        cout<<ans<<endl;

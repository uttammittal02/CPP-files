...
int32_t main(){
    ...
    // test_cases_loop{
        ...
        int a = input[0].ff, b = input[0].ss;
        sort(all(input));
        // int ans = max((int)0, input[n-1].ff - a - b) + max((int)0, a - input[0].ff - input[0].ss);
        int i = 0;
        while (i != n){
            int j;
        }
        // ans = min(ans, max(input[n-1].ff - input[0].ff - input[0].ss, (int)0));
        // sum += ans;
        cout<<sum<<endl;
    // }
    return 0;

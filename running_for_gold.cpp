...
int32_t main(){
	...
	test_cases_loop{
        int n; cin>>n;
        vector <vi > rankings(n, vi(5, 0));
        loop(i, 0, n){
            cin>>rankings[i][0]>>rankings[i][1]>>rankings[i][2]>>rankings[i][3]>>rankings[i][4];
        }
        int i = 0;
        bool ans = true;
        while (i < n){
            ...
            loop(j, i+1, n){
                int cnt = 0;
                loop(k, 0, 5){
                    if (rankings[j][k] < rankings[i][k]) cnt ++;
                }
                if (cnt >= 3) {i = j; break;}
            }
            if (temp == i){

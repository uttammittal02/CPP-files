...
int32_t main(){
	...
	test_cases_loop{
        string s; cin>>s;
        int l1(INT64_MAX), l0(INT64_MAX), r1(INT64_MIN), r0(INT64_MIN);
        // loop(i, 0, sz(s)){
        //     if (s[i] == '0') l0 = min(l0, i);
        //     else l1 = min(l1, i);
        //     if (s[sz(s) - i - 1] == '0') r0 = max(r0, sz(s) - i - 1);
        //     else r1 = max(r1, sz(s) - i - 1);
        // }
        // int ans = -1;
        // // cout<<l1<<space<<r1<<space<<l0<<space<<r0<<endl;
        // if (r1 < l1) ans = 1;
        // if (r0 < l0) ans = 0;
        // if (ans == -1){
        //     // cout<<max(l1, l0)<<space<<min(r1, r0)<<endl;
        //     if (max(l1, l0) > min(r1, r0)) ans = 1;
        //     else ans = 2;
        // }
        int ans = 0;
        int cnt(0);
        loop(i, 0, sz(s)){
            if (s[i] == '1' Or i == sz(s) - 1){
                if (s[i] == '0') cnt++;
                if (cnt) ans++;
                cnt = 0;
                continue;

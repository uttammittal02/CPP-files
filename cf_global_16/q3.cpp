...
int MEX(int i, int j){
    ...
    if (min(i, j) == 1) return 0;
    if (max(i, j) == 0) return 1;
    return 2;
}
int32_t main(){
    FIO
	test_cases_loop{
        ...
        if (n==1) {
            ans = MEX(row1[0] - '0', row2[0] - '0');
            cout<<ans<<endl; continue;
        }
        else {
            int i = 0;
            // while (i <n-1){
            //     ans += max(MEX(row1[i] - '0', row2[i] - '0') + MEX(row1[i+1] - '0', row2[i+1] - '0'), MEX(MEX(row1[i] - '0', row2[i] - '0'), MEX(row1[i] - '0', row2[i] - '0')));
            //     i+=2;
            // }
            // if (i== n-1){
            //     ans += MEX(row1[i] - '0', row2[i] - '0');
            // }
            while(i<n){
                // cout<<i<<" ans = "<<ans<<endl;
                if (!MEX(row1[i] - '0', row2[i] - '0')){
                    // cout<<row1[i]<<space<<row2[i]<<endl;
                    if (i==n-1) break;
                    if ((row1[i+1]- '0') And (row2[i+1] - '0')){
                        i++; continue;
                    }
                    ans += 2;
                    i+=2; continue;
                }
                else if (MEX(row1[i] - '0', row2[i] - '0') == 1){
                    if (i == n-1) {ans ++; i++; continue;}
                    if ((row1[i+1]- '0') And (row2[i+1] - '0')) {
                        ans += 2; i+=2; continue;
                    }
                    ans ++; i++; continue;

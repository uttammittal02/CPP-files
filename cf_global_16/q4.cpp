...
int32_t main(){
	...
	test_cases_loop{
        ...
        map <int, vi> mapping;
        mapping.clear();
        loop(i, 0, n*m) {cin>>arr[i]; mapping[arr[i]].pb(i);}
        sort(all(arr));
        int i = n*m-1;
        mii seating;
        while (i>=0){
            // seating[mapping[arr[i]]] = 
            int j = i;
            int row_st = i/n, col_end = i%n;
            int last = i- mapping[arr[i]].size() + 1;
            int row_last = last/n, col_last = last%n;
            while (arr[j] == arr[i] And j>=0){
                if (row_last != row_st){
                    // int k = n-1;
                    for (int row = row_last; row <= row_st; row ++){
                        int col = n;
                        while (col > 0){
                            // cout<<arr[j]<<space<<row<<space<<k<<endl;
                            col--;
                            if ((row == row_st And (col) > col_end) Or (row == row_last And (col) < col_last)){
                                continue;
                            }
                            seating[mapping[arr[i]][i-j]] = row*n + col;
                            // cout<<mapping[arr[i]][i-j]<<space<<row<<space<<(k)<<endl;
                            j--;
                        }
                        // k--;
                    }
        ...
        loop(i,0, n*m){
            int seat = seating[i];
            int row = seat / n;
            // cout<<i<<space<<seat%n<<space<<row<<endl; 
            loop(j, n*row, seat){
                if (visited[j]) {
                    ans++;
                    // cout<<"j "<<j<<endl;

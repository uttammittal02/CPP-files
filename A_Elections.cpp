...
int32_t main(){
    ...
    test_cases_loop{
	    int a, b, c; cin>>a>>b>>c;
        int cnt = 0;
        if (a == max(a, max(b, c))) cnt ++;
        if (b == max(a, max(b, c))) cnt ++;
        if (c == max(a, max(b, c))) cnt ++;
        cout<<max(a, max(b, c)) - a + 1 - bool(a == max(a, max(b, c)) and cnt == 1)<<space<<max(a, max(b, c)) - b+1 - bool(b == max(a, max(b, c)) and cnt == 1)<<space<<max(a, max(b, c)) - c + 1 - bool(c == max(a, max(b, c)) and cnt == 1)<<endl;
    }
    return 0;
}

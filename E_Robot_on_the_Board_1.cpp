...
int32_t main(){
    ...
    test_cases_loop{
        ...
        int vert(0), hor(0);
        loop(i, 0, sz(s)){
            // if ((maxis[0] + maxis[1] >= n-1) and (maxis[2] + maxis[3] >= m-1)) break;
            // cout<<vert<<" debug "<<hor<<endl;
            if (s[i] == 'U'){
                if (maxis[1] >= n-1 and vert == 0) break;
                if ((maxis[0] + maxis[1] >= n-1) and vert == maxis[0]) break;
                vert++;
                if (vert > 0)
                    maxis[0] = max(maxis[0], vert);
            }
            if (s[i] == 'D'){
                if (maxis[0] >= n-1 and vert == 0) break;
                if ((maxis[0] + maxis[1] >= n-1) and vert == (0 - maxis[1])) break;
                vert--;
                if (vert < 0)
                    maxis[1] = max(maxis[1], abs(vert));
            }
            if (s[i] == 'L'){
                if (maxis[3] >= m-1 and hor == 0) break;
                if ((maxis[2] + maxis[3] >= m-1) and hor == (0 - maxis[2])) break;
                hor--;
                if (hor < 0)
                    maxis[2] = max(maxis[2], abs(hor));
            }
            if (s[i] == 'R'){
                if (maxis[2] >= m-1 and hor == 0) break;
                if ((maxis[2] + maxis[3] >= m-1) and hor == maxis[3]) break;
                hor++;
                if (hor > 0)
                    maxis[3] = max(maxis[3], hor);
            }
        }
        cout<<maxis[0] + 1<<space<<maxis[2]+1<<endl;
    }
    return 0;
}
//UDLR

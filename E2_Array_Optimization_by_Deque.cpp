...
// int check(deque <int> &ans, vi &arr, int idx, int n, vi &dp, int min, int max){
//     if (arr[i])
// }
...
int32_t main(){
    ...
    test_cases_loop{
        ...
        loop(i, 0, n){
            ...
            cin>>temp;
            o_set.insert({temp, i});
            int smaller_ele = o_set.order_of_key({temp, -1});
            int greater_ele = i - o_set.order_of_key({temp, i});
            ans += min(smaller_ele, greater_ele);
        }
        cout<<ans<<endl;

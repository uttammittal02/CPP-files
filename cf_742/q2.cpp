...
// ------------------------------0 1 11 00 100 001 011 001 1001----------------------------------------------------------------------------- //

int32_t main(){
    FIO
    vi check_((int)1e6 + 5, 0);
    loop(i, 1, (int)1e6){
        check_[i] = check_[i-1] ^ i;
    }
	test_cases_loop{
        int a, b; cin>>a>>b;
        // cout<<"a, b "<<a<<space<<b<<endl;
        int check = check_[a-1];
        // cout<<"a "<<check<<endl;
        int copy = b, final_ans = 0;
        if (check == b){
            cout<<a<<endl; continue;
        }
        if (a == 1){
            if (b!=1) {cout<<2<<endl; continue;}
        ...
        // else{
        //     vi ans((int)log2(check) + 1);
        //     int n = (int)(log2(check)) + 1;
        //     loop(i,0, n){
        //         int curr_check = check%2; check/=2;
        //         int curr_bit = copy%2; copy/=2;
        //         // cout<<"debug "<<curr_check<<space<<curr_bit<<endl;
        //         if(curr_bit == curr_check) ans[i] = 0;
        //         else ans[i] = 1;
        //         final_ans += ((1<<i) * ans[i]);
        //     }
        // }
        // cout<<"final "<<final_ans<<endl;
        final_ans = check^b;
        if (final_ans == a){
            cout<<a+2<<endl;
        }

...
int32_t main(){
	...
	test_cases_loop{
        ...
        if (sz(sum) >= n){
            loop(i, 0, sz(sum)) {
                ans.pb((sum[i] - '0') * pow(10, sz(sum) - i - 1));
                if (!ans[sz(ans) - 1]){
                    ans.pop_back();
                }
                if ((sz(ans) == n - 1) And (i != sz(sum) - 1)){
                    stringstream temp = sum.substr(i+1, sz(sum) - i - 1);
                    if ( - '0'){
                        loop(i, 0, n - 1) cout<<ans[i] <<space;
                        cout<<sum.substr(i+1, sz(sum) - i - 1)<<endl;
                        continue;
                        }
                }
            }

...
int32_t main(){
    ...
    if (*lower_bound(all(brr), brr[n]/2.0) == brr[n]/2.0){
        // cout<<"debug "<<*lower_bound(all(brr), brr[n+1]/2.0)<<endl;
        cout<<(lower_bound(all(brr), brr[n]/2.0) - brr.begin())<<endl;
        exit(0);
    }
    cout<<upper_bound(all(brr), brr[n]/2.0) - brr.begin()<<endl;
    return 0;
}

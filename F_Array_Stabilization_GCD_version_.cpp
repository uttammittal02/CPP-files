...
int Sparse_table::lookuptable(int start, int length, int (*fun)(int, int)){
    ...
    // cout<<table[cnt][start]<<space<<table[cnt][end - x + 1]<<endl;
    // if (end < start){
    //     return fun(table[cnt][start], table[cnt][end - x + 1]);
    // }
...
int binary_search(Sparse_table table, int start, int n, int hcf){
    int ans = n;
    int mid = 1;
    // while(true){
        // if (table.lookuptable(start, mid, fun) != hcf) break;
        // int a = table.lookuptable(start, mid, fun);
        // mid*=2;
        // assert(table.lookuptable(start, mid, fun) != hcf);
    // }
    // cout<<"debug "<<table.lookuptable(start+1, n, fun)<<endl;
    int lo = 1, hi = n;
    while (lo <= hi){
        mid = lo + (hi - lo)/2;
        if (table.lookuptable(start+1, mid, fun) > hcf){
            lo = mid + 1;
        }
        else{
            hi = mid - 1;
...
int32_t main(){
    ...
    test_cases_loop{
        ...
        Sparse_table input(2*n);
        input.makeuptable(2*n, arr, fun);
        int ans = 0;
        loop(i, 0, n){
            ans = max(ans, binary_search(input, i, n, hcf));
        }
        cout<<ans-1<<endl;
    }
    return 0;

...
int32_t main(){
    FIO
    // string s = "abcba";
    // cout<<s<<" before  and then afterwards ";
    // s.erase('a'); cout<<s<<endl;
	test_cases_loop{
        string t, s; cin>>t;
        int n = sz(t);
        mii freq;
        ...
        string deleted = "";
        int no_of_char = sz(freq);
        int j = n-1;
        int i = n-1; 
        bool exist = true;
        vi visited(26, 0);
        while (i>=0){
            if (freq[t[i] - 'a'] % no_of_char) {
                // cout<<t[i] <<" 12344 "<<freq[t[i] - 'a']<<space<<no_of_char<<endl;
                exist = false; break;
            }
            freq[t[i] - 'a'] /= no_of_char;
            deleted += t[i];
            no_of_char--;
            visited[t[i] - 'a'] ++;
            if (!no_of_char) break;
            while(t[i] == t[j] Or visited[t[i] - 'a']){
                i--;
            }
            j = i;
        }
        ...
        loop(i, 0, sz(deleted)){
            // int j(0);
            check += s;
            s.erase(remove(s.begin(), s.end(), deleted[i]), s.end());
            // len -= freq[deleted[i] - 'a'];
            // cout<<check<<endl;
        }
        loop(i, 0, n){
            if(check[i] != t[i]){

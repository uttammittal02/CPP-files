...
int32_t main(){
    ...
    loop(i, 0, n){
        ...
        pii pair;
        pair.ff = *min_element(all(matrix[i]));
        pair.ss = *max_element(all(matrix[i]));
        if (pair.ff != pair.ss) ans++;
        if (!ascent[i]){
            min_max.pb(pair);
        }
    }
    ...
    int j;
    loop(i, 0, min_max.size() - 1){
        j = i+1;
        while (min_max[i].ff >= min_max[j].ss And j < min_max.size()){
            j++;
        }
        ans += max(sz(min_max) - j, (int)0);
    }
    for(int i = sz(min_max) - 1; i > 0; i--){
        j = i-1;
        while (min_max[i].ff >= min_max[j].ss And j >= 0){
            j--;
        }
        ans += max((int)0, j+1);
    }

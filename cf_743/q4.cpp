...
int32_t main(){
	...
	test_cases_loop{
        ...
        // }
        int cnt = 0;
        while (!ones.empty() And sz(ans) < n){
            bool found = false;
            set<int>::iterator ptr;
            for (ptr = copy.begin(); ptr != copy.end(); ptr++){
                cnt ++;
                int idx = *ptr;
                if (ones.find(idx) == ones.end()) continue;
                if (ones.find(idx+1) != ones.end()){
                    // ones.erase(idx);
                    if (idx And ones.find(idx-1) == ones.end()){
                        ans.pb(idx - 1);
                        // arr[idx] = 0;
                        // arr[idx+1] = 0;
                        ones.erase(idx);
                    ...
                    else if (idx + 2 < n And ones.find(idx+2) == ones.end()){
                        ans.pb(idx);
                        // arr[idx] = 0;
                        // arr[idx+1] = 0;
                        ones.erase(idx);
                ...
                else if (ones.find(idx + 2) != ones.end() And ones.find(idx + 1) == ones.end()){
                    ans.pb(idx);
                    found = true;
                    ones.erase(idx);
                    ones.erase(idx+2);

...
int32_t main(){
	...
	int n, q;
    cin>>n>>q;
    vi strength(n), pref_str(n+1, 0), attack(q), pref_attack(q+1, 0), ans(q, 0);
    loop(i, 0, n){
        cin>>strength[i];
        pref_str[i+1] = pref_str[i] + strength[i];
    }
    loop(i, 0, q){
        cin>>attack[i];
        pref_attack[i+1] = pref_attack[i] + attack[i];
        if (pref_attack[i+1] >= pref_str[n]){
            ans[i] = n;
            pref_attack[i+1] = 0;
        }
        // pref_attack[i+1]%= pref_str[n];
    }
    loop(i, 0, q){
        if (!ans[i]){
            ans[i] = pref_str.end()- upper_bound(all(pref_str), pref_attack[i+1]);
        }
        cout<<ans[i]<<endl;
    }
    return 0;

...
int32_t main(){
    FIO
	int length_of_199s_array; cin>>length_of_199s_array;
    vi arr199(length_of_199s_array);
    loop(i, 0, length_of_199s_array)cin>>arr199[i];
    bool done = true;
    loop(i,0 , length_of_199s_array-1){
        int min_ = INT64_MAX;
        int idx = i;
        // idx = min_element(arr199.begin() + i, arr199.end()) - arr199.begin(); 
        //inbuilt function of cpp that can perform function of loop from line 138 to 144.
        loop(j, i, length_of_199s_array){
            // min_ = min(min_, arr[j]);
            if (arr199[j] < min_){
                min_ = arr199[j];
                idx = j;
        ...
        loop(i, 0, length_of_199s_array){
            //This loop prints the array after each operation
            cout<<arr199[i]<<space;
            if (i And arr199[i] < arr199[i-1]){
                done = false; break;
                // this checks if the array is sorted or needs further operations to sort it.
            }
        }
        if (done) break;
        cout<<endl;
    ...
    if (length_of_199s_array == 1){
        cout<<arr199[0]<<space; // since the array length is 1 so, the only element in the array is 0th element.
    }
    return 0;
}

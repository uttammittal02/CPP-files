...
int32_t main()
{
    ...
    std::uniform_int_distribution<> distr(1, 2 * pec_199); // define the range
    ...
    {
        bool done = true;
        int idx_min_element = min_element(uttam.begin() + i, uttam.end()) - uttam.begin();
        // swap(uttam[i], uttam[idx_min_element]);
        for (int j = idx_min_element; j > i; j--)
            swap(uttam[j], uttam[j - 1]);
        
        loop(i, 0, pec_199)
        {
            if (i And uttam[i] < uttam[i - 1])
                done = false;

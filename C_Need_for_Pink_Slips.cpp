...
#define ff                                          first
#define ss                                          second
#define precision(y, x)                             fixed<<setprecision(x)<<y     
using namespace std;
...
//--------------------------------------------------------------------------------------------------------//
float v;
float dfs(float c, float m, float p, int numb_of_rounds){
    // cout<<(bool) (p == (float)1)<<" p = 1\n";
    cout<<numb_of_rounds<<space<<c<<space<<m<<space<<p<<endl;
    if (p == (float)1) return (float)numb_of_rounds;
    
    float ans = 0;

    if (c != (float)0){
        ans += c * dfs(max((float)0, c-v), m + (bool)(m!=0) * min(c, v) / 2, p + min(c, v) * (m == 0? 1 : 0.5), numb_of_rounds + 1);
    }
// 0.466
    if (m != (float)0){
        ans += m * dfs(c + (bool)(c!=0) * min(m, v) / 2,max((float)0, m-v), p + min(m, v) * (c == 0? 1 : 0.5), numb_of_rounds + 1);
    }

    ans += p * numb_of_rounds;
    // cout<<"ans "<<ans<<endl;
...
int32_t main(){
    FIO
    test_cases_loop{
	    float c, m, p; cin>>c>>m>>p>>v;
        // cout<<c<<m<<p<<v<<endl;
        float ans = 0;
        // int cnt = 0;
        ans += dfs(c, m, p, 1);
        cout<<precision(ans, 12)<<endl;
    }
    return 0;
}

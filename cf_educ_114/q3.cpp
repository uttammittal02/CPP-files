...
int32_t main(){
    ...
    loop(i, 0, m){
        ...
        if (idx){
            ans += min(max((int)0, y - (sum - arr[idx])) + max((int)0, (x - arr[idx])), max((int)0, y - (sum - arr[idx-1])) + x - arr[idx-1]);
        }
        else ans = max((int)0, y - (sum - arr[idx])) + max((int)0, (x - arr[idx]));
            // if (arr[idx]!=x){
                 
            //     if (idx) idx --;

            //     ans += max((int)0, (x - arr[idx]));
            // }
        cout<<ans<<endl;
    }
    // }

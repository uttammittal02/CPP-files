...
int32_t main(){
	...
	test_cases_loop{
        ...
        bool var = false;
        // if (sz(unique) <= k) {         5 3
        //     cout<<1<<endl; continue;   1 2 3 4 5        3
        // }                              1 2
        set<int>::iterator itr;
        loop(m, 1, 100){
            int numb1(INT64_MIN), numb2(INT64_MIN);
            int cnt = 0;
            for (itr = unique.begin(); itr != unique.end(); itr++){
                int a = *(itr);
                if (a%m) {
                    if (a/m == numb1) cnt--;
                    else if (a/m == numb2) cnt--;
                    if (a/m == numb2 -1) cnt--;
                    numb2 = (a/m + 1), cnt+=2;
                    numb1 = a/m;
                    continue;
                }
                if (a/m == numb1 Or a/m == numb2) cnt--;
                cnt++;
                numb1 = a/m;
            }
            if (cnt <= k){
...
// --------------------------------------0 1 1 2 2--------0 0 0 2 4 5 7 7 7------------------------------------------------------------- //
// --------------------------------------1 1 2 2 3--------2 2 3 3 3 6 6 6 10------------------------------------------------------------- //

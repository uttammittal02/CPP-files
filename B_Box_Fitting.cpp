...
int32_t main(){
    ...
    test_cases_loop{
        ...
        sort(all(widths), greater<int>());
        int cnt = 0;
        // vi blocks;
        multiset <int, greater<int> > blocks;
        blocks.insert(w);
        loop(i, 0, n){
            int available_space = *(blocks.begin());
            if (widths[i] <= available_space){
                blocks.erase(blocks.begin());
                blocks.insert(available_space - widths[i]);
            }
            else
                blocks.insert(w - widths[i]);
            // cout << *(blocks.begin()) << space << *(--blocks.end()) << endl;

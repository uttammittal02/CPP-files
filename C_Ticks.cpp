...
// bool check(vector <string> &matrix, vector <vi > &visited, int rows, int cols, int k, int i, int j, bool down, int cnt){
//     visited[i][j] = 1;
//     bool ans = false;
//     if (down){
...
//             if (cnt >= k){
//                 if (matrix[i-1][j+1] == '*'){
//                     ans = ans or check(matrix, visited, rows, cols, k, i-1, j+1, 0, cnt);
//                 }
//             }
//             i++;j++;
//         }
...
int32_t main(){
    ...
    // test_cases_loop{
	//     int no_of_rows, no_of_cols, k; cin>>no_of_rows>>no_of_cols>>k;
    //     vector <string> matrix(no_of_rows);
    //     loop(i, 0, no_of_rows){
    //         cin>>matrix[i];
    //     }
    //     set<pii> visited;
    //     set<pii> back_tracker;
    //     loop(i, 0, no_of_rows){
    //         loop(j, 0, no_of_cols){
    //             if (matrix[i][j] == '*'){
    //                 pii p1; 
    ...
    //                 p1.ff = i, p1.ss = j;
    //                 int x1 = i-1, y1 = j-1, y2 = j+1;
    //                 visited.insert(p1);
    //                 int cnt = 0;
    //                 pii po, p2;
    //                 while (x1>=0 And y1>=0 And y2 < no_of_cols And matrix[x1][y1] == '*' And matrix[x1][y2] == '*'){
    //                     cnt++;
    //                     po.ff = x1, po.ss = y1;
    //                     p2.ff = x1, p2.ss = y2;
    //                     x1--, y1--, y2++;
    ...
    //                     visited.insert(p2);
    //                 }
    //                 if (cnt < k) visited = back_tracker;
    //                 else back_tracker = visited;
    //                 // cout<<i<<space<<j<<space<<cnt<<space<<(bool)(visited.find(p1) != visited.end())<<endl;
    //             }
    //         }
    //     }
    //     bool wrong = false;
    ...
    //             pii pair;
    //             pair.ff = i, pair.ss = j;
    //             if(matrix[i][j] == '*' And visited.find(pair) == visited.end()){
    //                 wrong = true; 
    //                 break;
    //             }
    //         }

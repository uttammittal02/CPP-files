...
int32_t main(){
	...
	test_cases_loop{
        int n, ones(0); cin>>n;
        vi arr(n), brr(n), first, second;
        loop(i, 0, n){
            cin>>arr[i];
            arr[i] %=2;
            if (arr[i])ones++, first.pb(i);
            else second.pb(i);
            if (!i){
                brr[i] = arr[i];
            }
        ...
        int swaps1(0), swaps2(0), odd(0);
        if (ones > zeros){
            loop(i, 0, first.size()){
                swaps1 += abs(first[i] - odd);
                swaps2 += abs(first[i] - odd - 1);
                if (odd + 1 >= n){
                    swaps2 = INT64_MAX;
                } 
                odd+=2;
            ...
            loop(i, 0, second.size()){
                swaps1 += abs(second[i] - odd);
                swaps2 += abs(second[i] - odd - 1);
                if (odd + 1 >= n){
                    swaps2 = INT64_MAX;
                } 
                odd+=2;

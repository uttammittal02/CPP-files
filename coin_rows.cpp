...
int32_t main(){
	...
	test_cases_loop{
        ...
        vi suffix1(m+1, 0), prefix2(m+1, 0);
        // prefix2[1] = row2[0];
        loop(i, 0, m){
            int idx = m-i-1;
            // if (i) suffix2[idx]  = suffix2[idx + 1] + row2[idx];
            prefix2[i+1] = prefix2[i] + row2[i];
            suffix1[idx] = suffix1[idx+1] + row1[idx]; 
        }
        
        int bob = INT64_MAX;
        loop(i, 0, m){
            int potential_answer = max(prefix2[i], suffix1[i+1]);
            bob = min(bob, potential_answer);
        }
        cout<<bob<<endl;
    }
    return 0;

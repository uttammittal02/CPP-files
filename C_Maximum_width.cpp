...
int func(string &s, string &t, int n, vector<vi> &dp, int idx, int s_idx){
    if (idx >= m) return 0;
    if (idx == m-1) return 0;
    if (dp[idx][0] == -1){
        int cnt = 0;
        while (s[s_idx+cnt] != t[idx]){ 
            cnt++;
            if (s_idx + cnt >= n) 
                return INT64_MAX;
        }
        dp[idx][0] = max(cnt, func(s, t, n, dp, idx+1, s_idx+1));
    }
    if (dp[idx][1] == -1){
        int i = n-1;
        while (i >= s_idx and s[i] != t[idx]) {
            s_idx++;
        }
        dp[idx][1] = func(s, t, n, dp, idx+1, s_idx+1);

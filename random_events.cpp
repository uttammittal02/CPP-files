...
void precision_print(double n, int p){
...
int32_t main(){
	...
	test_cases_loop{
        int n, m;
        cin>>n>>m;
        vi input(n+1, 0), sorted(n+1, 0);
        loop(i, 1, n+1) cin>>input[i], sorted[i] = input[i];
        sort(all(sorted));
        int reqd_idx = 0;
        for (int i = n; i>=0; i--){
            if (sorted[i] != input[i]){
        ...
        // cout<<"dbeug "<<reqd_idx<<endl;
        double ans = 1;
        loop(i, 0, m){
            int r; float p;
            cin>>r>>p;
            if (r >= reqd_idx) ans *= (1-p);
        }
        if (!reqd_idx) {
            cout<<"1.000000"<<endl; continue;
        }
        // float b = ans.precision(6);
        precision_print(1 - ans, 6);
    }
    return 0;
}
